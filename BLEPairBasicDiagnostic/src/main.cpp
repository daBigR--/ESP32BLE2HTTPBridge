...
class SecurityCB : public NimBLESecurityCallbacks {
    ...
    void onPassKeyNotify(uint32_t pass_key) override {
        Serial.printf("[SECURITY]  Passkey notify: %06u\n", pass_key);
    }
    ...
    bool onSecurityRequest() override {
        Serial.println("[SECURITY]  Security request accepted");
        return true;
    }
...
static NimBLEAdvertisedDevice* scanForTargetAdvert(const NimBLEAddress& address)
{
    NimBLEAdvertisedDevice* target = nullptr;

    class TargetScanCB : public NimBLEAdvertisedDeviceCallbacks {
    public:
        TargetScanCB(const NimBLEAddress& targetAddr, NimBLEAdvertisedDevice** ppTarget)
          : address(targetAddr.toString()), ppTarget(ppTarget) {}

        void onResult(NimBLEAdvertisedDevice* dev) override {
            if (!dev) return;
            if (dev->getAddress().toString() != address) {
                return;
            }

            bool foundConnectable = (
                dev->getAdvType() == BLE_HCI_ADV_RPT_EVTYPE_ADV_IND ||
                dev->getAdvType() == BLE_HCI_ADV_RPT_EVTYPE_DIR_IND
            );

            if (!*ppTarget) {
                *ppTarget = new NimBLEAdvertisedDevice(*dev);
            } else if (!(*ppTarget)->isConnectable() && foundConnectable) {
                delete *ppTarget;
                *ppTarget = new NimBLEAdvertisedDevice(*dev);
            }
...
static bool connectAndExplore()
{
    ...
    NimBLEAdvertisedDevice* target = scanForTargetAdvert(targetAddress);
    bool ok = false;

    if (target) {
        Serial.printf("[CONN]  Target advert found; addr=%s type=%d advType=0x%02X connectable=%s\n",
                      target->getAddress().toString().c_str(),
                      target->getAddressType(),
                      target->getAdvType(),
                      target->isConnectable() ? "yes" : "no");
        Serial.println("[CONN]  Attempting connect via advertised device record");
        delay(100);
        ok = pClient->connect(target, false);
        if (!ok) {
            int t = target->getAddressType();
            Serial.printf("[CONN]  connect(target) failed rc=%d (%s)\n",
                          pClient->getLastError(),
                          NimBLEUtils::returnCodeToString(pClient->getLastError()));
            if (t >= 0) {
                Serial.printf("[CONN]  Retrying with explicit address type %d\n", t);
                ok = pClient->connect(NimBLEAddress(target->getAddress().toString().c_str(), t), false);
            }
        }
        delete target;
    }
    ...
    if (!ok) {
        Serial.println("[CONN]  Falling back to direct address connect attempts");
        ok = pClient->connect(NimBLEAddress(targetAddress.toString().c_str()), false);
        if (!ok) {
            Serial.printf("[CONN]  direct generic failed rc=%d\n", pClient->getLastError());
            ok = pClient->connect(NimBLEAddress(targetAddress.toString().c_str(), BLE_ADDR_RANDOM), false);
        }
        if (!ok) {
            Serial.printf("[CONN]  RANDOM failed rc=%d\n", pClient->getLastError());
            ok = pClient->connect(NimBLEAddress(targetAddress.toString().c_str(), BLE_ADDR_PUBLIC), false);
        }
        if (!ok) {
            Serial.printf("[CONN]  PUBLIC failed rc=%d\n", pClient->getLastError());
            ok = pClient->connect(NimBLEAddress(targetAddress.toString().c_str(), BLE_ADDR_RANDOM_ID), false);
        }
        if (!ok) {
            Serial.printf("[CONN]  RANDOM_ID failed rc=%d\n", pClient->getLastError());
            ok = pClient->connect(NimBLEAddress(targetAddress.toString().c_str(), BLE_ADDR_PUBLIC_ID), false);
        }
    }

    if (!ok) {
        Serial.printf("[CONN]  Connect failed, final rc=%d (%s)\n",
                      pClient->getLastError(),
                      NimBLEUtils::returnCodeToString(pClient->getLastError()));
        NimBLEDevice::deleteClient(pClient);
        pClient = nullptr;
...
void setup()
{
    ...
    NimBLEDevice::setSecurityCallbacks(&securityCB);
